...
function Weather(weather: any) {
	if (weather == "Sunny") return <img width="200" height="200"
												   src="https://icons.iconarchive.com/icons/icons-land/weather/256/Sunny-icon.png"/>
	else if (weather == "Cloudy") return <img width="200" height="200"
														 src="https://cdn.iconscout.com/icon/free/png-256/cloudy-1602000-1358407.png"/>
	else if (weather == "Rainy") return <img width="200" height="200"
...
export default function Home() {
    ...
    const validateWeather = function() {
        missionAPI.modifyPoll("weather", "true")
            .then(res => {
                setPoll(Object.assign(new Poll(), res.data))
    ...
    const validateRocket = function() {
        missionAPI.modifyPoll("rocket", "true")
            .then(res => {
                setPoll(Object.assign(new Poll(), res.data))
  ...
  return (
    ...
    <div className={classes.root}>
	  ...
	  <h2>Weather department</h2>
		...
		<br/>
		<Button variant="contained" color="secondary" onClick={validateWeather}>
	...
	<Button variant="contained" color="primary" onClick={launchBooster}>
		Launch Booster
	</Button>
	<Button variant="contained" color="primary" onClick={destroyBooster}>
		Destroy Booster
	</Button>
      </TabPanel>
	  <TabPanel value={value} index={3}>
	  <h2>Mission department</h2>
	<Button variant="contained" color="secondary" onClick={createPoll}>
		Create the poll
	</Button>
	<br/>
	<Button variant="contained" color="primary" onClick={getPoll}>
		Refresh the poll
	</Button>
	{poll !== undefined &&
	<>
		<p>Weather status : {poll?.weatherStatus ? "OK" : "NO"}</p>
		<p>Rocket status : {poll?.rocketStatus ? "OK" : "NO"}</p>
		<p>Mission status : {poll?.missionStatus ? "OK" : "NO"}</p>
	</>
	}

	<br/>
	<Button variant="contained" color="secondary" onClick={validateMission}>
		Validate on poll
	</Button>
      </TabPanel>
	  <TelemetryMonitor/>
